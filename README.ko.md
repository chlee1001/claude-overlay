...
```bash
./doctor.sh                             # 읽기전용 통합 헬스 판정 (업데이트 후 가장 먼저 이거)
./doctor.sh --quiet                     # 한 줄 판정 + 종료 코드, 오프라인 안전 (--local-only reabsorb)
...
`doctor.sh`는 구조상 읽기전용이다 — `--write` 경로 자체가 없다 — `apply.sh` + `reabsorb.sh`
+ 버전 스큐 표시(활성 캐시 vs 마켓플레이스 클론)를 하나의 판정과 종료 코드로 합친다(`3` 오류 ·
`2` 하드=CONFLICT/MISSING · `5` 소프트 드리프트 · `4` 정보 · `0` 정상). `--quiet`는 git-repo
네트워크 프로브를 건너뛰므로 깨끗해도 종료 `4`가 정상이고, 진짜 all-green 확인은 전체 `./doctor.sh`
= `0`이다. 심각도 표는 [SKILL.md](SKILL.md)의 "Health check" 절 참고.
...
claude-overlay/
  ...
  doctor.sh                      # 읽기전용 통합 헬스 판정 (apply.sh + reabsorb.sh + 버전 스큐)
  lib/omc-version.sh             # 버전 조회 + 스큐 표시 공용 헬퍼 (apply.sh + doctor.sh가 사용)
  ...
  REGRESSIONS.md                 # 화석화된 버그 대장; 각 줄이 tests/run.sh의 영구 픽스처를 가리킴
