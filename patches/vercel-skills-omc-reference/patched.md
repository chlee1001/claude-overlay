...
<!-- VERCEL_SKILLS_ROUTING_TABLE -->
## Vercel skills — when → which (absorbed lazy skills)

| Situation | Skill | Lane |
|---|---|---|
| Writing/refactoring React/Next.js (perf, data-fetch, bundle) | `vercel-react-best-practices` | executor |
| React component composition (React 19) | `vercel-composition-patterns` | executor; architect may reference |
| Route / page / shared-element transitions | `vercel-react-view-transitions` | executor |
| React Native | `vercel-react-native-skills` | executor |
| Reviewing UI code / accessibility / design audit | `vercel-web-design-guidelines` | code-reviewer (REVIEW, not design-time) |

Lane notes:
- `executor` and `code-reviewer` are explicitly patched to `Skill()`-invoke these on domain match.
- `architect` (READ-ONLY): may reference `vercel-composition-patterns` when advising React structure — a light pointer, not a required load.
- `designer`: `vercel-composition-patterns` is relevant; note `vercel-web-design-guidelines` is a REVIEW skill (its real lane is `code-reviewer`), not a design-time generator.
<!-- /VERCEL_SKILLS_ROUTING_TABLE -->
