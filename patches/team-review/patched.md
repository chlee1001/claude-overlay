...
| **team-verify** | `verifier` (sonnet), `code-reviewer` (opus) | `test-engineer` (sonnet), `security-reviewer` (sonnet)                          | Always run `verifier` AND `code-reviewer` on every verify pass (code-reviewer is no longer optional). `code-reviewer` enforces spec-compliance (Stage 1) BEFORE code-quality (Stage 2) — see Per_Task_Two_Stage_Review below. Add `security-reviewer` for auth/crypto changes.          |
...
### Per_Task_Two_Stage_Review (mandatory)

Every `team-verify` pass MUST run `code-reviewer` over the work the preceding `team-exec`
pass produced — not only on large/architectural changes. `code-reviewer` already enforces a
two-stage order (defined in its agent spec): **Stage 1 spec-compliance** ("does this cover all
requirements; would the requester recognize it as their request?") must PASS before **Stage 2
code-quality** (SOLID, diagnostics, performance, style). Treat Stage 1 failures as blocking:
route them to `team-fix` before any Stage 2 nitpicks.

Scope it to the current pass's changed files (keep it cheap): review what `team-exec` just
touched, not the whole tree. In cost-downgrade mode `code-reviewer` may drop opus→sonnet, but
it still runs every pass — never skip it.
...
- **team-verify**
  ...
  - Agents: `verifier` + `code-reviewer` (always; Stage 1 spec → Stage 2 quality, see Per_Task_Two_Stage_Review) + task-appropriate reviewers (see routing table).
