...
## Deterministic Per-Dimension Floor (di-floor hook)

<!-- di-floor: honor deterministic per-dimension floor verdict before crystallize -->
A local overlay hook (`omc-di-floor.mjs`, PostToolUse on `state_write`) deterministically re-checks
per-dimension clarity floors after each scoring round, reading `topology.components[].clarity_scores`
(keys `goal/constraints/criteria/context`, per-dimension min across active components). Floors:
goal 0.75, constraints 0.65, criteria 0.70, context 0.60 (context only for brownfield). When the
weighted ambiguity is already ≤ threshold but a resolved dimension is still below its floor, the hook
injects a `<system-reminder>` naming the failing dimension(s). **Honor that verdict:** treat the
interview as NOT crystallize-ready regardless of the weighted score, and aim the next question at the
named dimension before returning here. The hook is fail-safe — absent/`null`/unscored dimensions are
skipped, never blocked — so a reminder means a genuinely scored dimension is weak.
