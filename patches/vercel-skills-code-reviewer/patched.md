...
<!-- VERCEL_SKILLS_ROUTING_REVIEWER -->
<Vercel_Skill_Routing>
  When reviewing React / Next.js diffs, load `Skill("vercel-react-best-practices")` and check the diff
  against its performance rule checklist. For UI / frontend diffs, also run
  `Skill("vercel-web-design-guidelines")` as a design + accessibility audit (it fetches the latest
  ruleset at review time). Skip for backend / CLI diffs.
</Vercel_Skill_Routing>
<!-- /VERCEL_SKILLS_ROUTING_REVIEWER -->
