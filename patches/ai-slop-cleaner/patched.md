...
3. **Classify the slop before editing**
   ...
   - **Comment/annotation slop** — comments that mislead or obscure instead of explaining (see the Comment Hygiene Checklist). Three sub-kinds:
     - *Unnatural Korean* — comments in awkward, AI-flavored Korean. Fix against the `writing-tropes.md` rules to a **terse** bar (clear and short), not literary prose. Do **not** run the full humanize-korean orchestrator on code — it is prose-only, normalizes code identifiers, and over-flowers terse comments.
     - *Planning-artifact leakage* — comments that reference implementation-plan scaffolding meaningless to a future reader ("Phase 0", "G1", "P2 fallback", "V1-V4", plan step IDs). The code shipped; the plan context did not.
     - *Cryptic abbreviations* — undefined abbreviations or jargon in comments that a later developer cannot decode. (Identifier naming stays in Pass 3, which is behavior-sensitive.)
...
Comments are non-executable, so rewriting one is behavior-safe **by construction** — except a bounded set of load-bearing comments that must never be edited.

- **Load-bearing denylist (never touch):** `eslint-disable`, `@ts-ignore` / `type: ignore`, `noqa`, `nolint`, `prettier-ignore`, `go:generate`, shebangs (`#!`), encoding declarations (`# -*- coding`), JSDoc/docstrings that drive typegen or `help()`/API docs, framework annotations, and doctests.
- **Deletion-first is the tiebreaker, not a conflict:** a comment that merely restates the code → delete; a planning-leak → delete (or rewrite to the real invariant); an awkward-Korean comment that carries a genuine "why" → rewrite terse.
- **No-hallucination rule (most important):** if a comment's intent cannot be verified from the code, **delete rather than rewrite** — deletion never invents intent, a rewrite can.
- **Preserve verbatim:** proper nouns, numbers, and any referenced IDs; add no rhetoric; never smuggle a code change into a comment-only diff.
- **Tests are blind here:** regression tests cannot catch comment correctness, so the reviewer pass (`--review`) is the gate for comment edits, not the test suite.

**Planning-leak detection (#2):** the regex only generates candidates; the classifier is a repo-resolvability check.

- Candidates (inside comment text only): `Phase\s?\d+`, `[GPVST]\d{1,2}` (G1/P2/V4), `V\d\s*-\s*V\d`, `\d+단계`, `Step\s?\d+`, `as (planned|discussed|per the plan)`.
- Keep (legit — resolves to a durable target): ticket IDs `[A-Z]{2,10}-\d+`, `#\d+`, `RFC \d+`, `CVE-…`, `ADR-\d+`, URLs, file paths.
- **Decisive test:** grep the token across the repo. If it appears only in comments and resolves to no spec/doc/test/tracker, it is leaked slop. Regex alone must never auto-delete — `G1`/`P2`/`S3` are also hardware pins, grid cells, and bucket names.
...
4. **Run one smell-focused pass at a time**
   ...
   - **Pass 5: Comment hygiene** — runs LAST, after Pass 4, against the final code so a comment is never rewritten and then orphaned by a later deletion. Apply the Comment Hygiene Checklist: delete-first, honor the load-bearing denylist, and route unnatural-Korean fixes through `writing-tropes.md` at a terse bar.
