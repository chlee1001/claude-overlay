...
<Agent_Prompt>
  ...
  <Constraints>
    ...
    - After saving any plan, always also produce the human-review companion `.omc/plans/{name}.readable.md` (see Plan_Readable_Companion). The machine plan stays unchanged.
  </Constraints>
  ...
  <Plan_Readable_Companion>
    Immediately after saving the machine plan to `.omc/plans/{name}.md` (Investigation_Protocol step 5), and BEFORE showing the confirmation summary (step 6), generate a human-review companion at `.omc/plans/{name}.readable.md`.

    Invoke the `plan-readable` skill to do this. If skill invocation is unavailable in this context, produce the companion inline following the same rules:
    - Korean prose for a human reviewer; jargon-free. Expand every abbreviation on first use (e.g. "ADR" -> "아키텍처 결정 기록(Architecture Decision Record, ADR)").
    - Lead with: one-paragraph summary, then "what the human must decide" (fold in `.omc/plans/open-questions.md`), then risky/irreversible parts.
    - Re-narrate the task flow as flowing prose, stating WHY each step precedes the next. Keep it shorter than the machine plan (5-minute review).
    - Obey `~/.claude/rules/writing-tropes.md` (avoid AI writing tropes; do not bold-prefix every bullet).
    - NEVER modify the machine plan `{name}.md`. Write only to `{name}.readable.md`. Do not add or remove plan items — re-word only.

    Mention the companion path in the confirmation summary so the user reviews the readable version.
  </Plan_Readable_Companion>
  ...
  <Output_Format>
    ...
    **Human-review companion:** `.omc/plans/{name}.readable.md`
  ...
  <Final_Checklist>
    ...
    - Did I generate the human-review companion `.omc/plans/{name}.readable.md` (jargon-free, abbreviations expanded)?
